...
Na začiatok je potrebné vytvoriť vhodný formát datasetu, na ktorom sa neurónová sieť  bude trénovať. Tento dataset  sa skladá z množiny obrázkov, ktoré predstavujú postavu vodiča a bod reprezentujúci hlavu. Pre jednotný format sa  detekovaná postava vykreslí na čierne pozadie a uloží ako miniatúrny obrázok. Veľkosť obrázka závisí aj na rýchlosti trénovania. Z tohto dôvodu bola šírka zvolená na 64 pixelov a výška  sa automaticky prispôsobí podľa tvaru postavu. Tieto obrázky boli zbierané z viacerých videí  a rozdlené do 3 spomínaných tried. Ukážku jednotlivých datasetov je možné vidieť  na obrázkock \ref{fig:NNSteering}, \ref{fig:NNShifting}  a \ref{fig:NNWrong}. Takýmto spôsobom bolo zozbieraných 11 957  trénovacích obrázkov. Rozpis pre jednotlivé triedy je  znázornený v tabuľke \ref{tab:nn}.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{Figures/NNsteering.png}
	\caption{Ukážka trénovacieho datasetu - držanie volantu}
...
\begin{figure}[H]
	...
	\caption{Ukážka trénovacieho datasetu - používanie radiacej páky}
...
\begin{figure}[H]
	...
	\caption{Ukážka trénovacieho datasetu - nesprávna pozícia}
	\label{fig:NNWrong}
\end{figure}
...
\end{tabular}
	\caption{Počet trénovacích obrázkvo pre jednotlivé triedy}
	\label{tab:nn}
\end{table}
...
\newpage
Po úspešnom vytvorení testovacieho datasetu je potrebné  sieť správne natrénovať. Tento krok je asi najdôležitejší a zlé natrénovanie siete vedie k  neúspechu celej detekcie. Je potrebné zvoliť správne vrstvy a ich počet, nakoľko sa  od toho odvíja úspešnosť natrénovania siete. Aby sa priebežne testovala úspešnosť siete, je vhodné  z datasetu určité percento  obrázkov zvoliť na testovanie. Z tohto dôvodu bola zvolená hodnota 10\%, ktorá je pre informatívne účely postačujúca. Aj keď sa na prvý pohľad môže zdať, že trénovacie vstupy vyzerajú rovnako, líšia sa v určitých typických veciach, ktoré sú typické pre celú triedu. Jedným z takýchto znakov je napríklad umiestnenie pravej ruky pri porovnaní  medzi datasetom na obrázku \ref{fig:NNSteering} a \ref{fig:NNShifting}. Aby sa všetky tieto znaky identifikovali správne, je potrebné pridať niekoľko skrytých vrstiev, ktoré tieto znaky zachytia. Model siete sa skladá zo vstupnej vrstvy,  4 skrytých vrstiev a výstupnej vrstvy:
\begin{itemize}
\item \textbf{Vstupná vrstva} - jedná sa o prvú vrstvu na spracovanie trénovacích obrázkov. Na vstupe bola použitá konvolučná vrstva, ktorá sa skladá z 32 filtrov s veľkosťou 3$\times$3. Jej hlavnou úlohou je previesť fyzický obrázok na  dáta, ktoré su schupné spracovať nasledujúce vrstvy siete.
\item \textbf{1. skrytá vrstva}  - predstavuje konvolučnú vrstvu s použitím 64 filtrov s rovnakou veľkosťou ako pri predchádzajúcej vrstve. 
\item \textbf{2. skrytá vrstva} -  jedná sa o redukovaciu vrstvu, ktorej hlavnou úlohou je redukcia dimenzií z predchádzajúcich vrstiev. Tieto dimenzie sa môžu redukovať na základe  rôznych hodnôt. Pri vytvárani modelu siete bola použitá vrstva \textit{MaxPooling2D} s veľkosťou filtra 2$\times$2. Ukážka fungovania redukovacej vrstvy je znázornená na obrázku \ref{fig:pooling}
...
\item \textbf{Výstupná vrstva} - výstupná vrstva sa skladá z 3 filtrov, ktoré reprezentujú jednotlivé triedy objektov s príslošnou hodnotou. Ako aktivačná funkcia je použitá funkcia \textit{SoftMax}, Softmax je veľmi zaujímavá aktivačná funkcia, pretože nielen mapuje  výstup do rozsahu $[0,1]$, ale tiež mapuje každý výstup tak, že celkový súčet je 1. Výstupom Softmax je preto rozdelenie pravdepodobnosti jednotlivých tried objektov
\end{itemize}
\newpage
\begin{lstlisting}[language=Python,label=src:cameraMatrix,caption={Vrstvy neurónovej siete}]
...
Po vytvorení modelu je na čase začať s trénovaním siete. Je potrebné zvoliť  správne množstvo iterácii (angl. epoch), aby sa model správne natrénoval. Ak by tento parameter bol príliš nízky, znamenalo by to nedostatočné natrénovanie a presnosť siete by nebola dostatočne vysoká. Naopak, po určitom počte  trénovacich iterácií je možné pozorovať určitú stágnáciu v úspešnosti siete a v  parametri straty (angl. loss).  Pokiaľ sa  tieto parametre novou iteráciou   nemenia výraznejším tempom, je potrebné trénovanie ukončiť.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{Figures/pooling.png}
	\caption{MaxPooling2D}
	\label{fig:pooling}
\end{figure}
